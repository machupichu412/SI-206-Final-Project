...
def avg_net_worth_by_vis(cur, conn):
    genres = get_genre_list(cur, conn)
    max_names = []
    max_net_worths = []
    min_names = []
...
def age_vs_net_worth(cur, conn):
    cur.execute("SELECT artists.net_worth, artists.age, artists.name FROM artists WHERE artists.age != -1 AND artists.age IS NOT NULL4 AND artists.net_worth != -1 AND artists.net_worth IS NOT NULL")
...
def main():
    ...
    cur = conn.cursor()
    valid = True
    userInput = int(input("Enter a visualization option (1-4): "))
    while valid:
        if userInput == 1:
